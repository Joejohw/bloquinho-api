...
Copy the local values from `.env.example` into your shell, run `docker compose up -d postgres`, then run `mvn test`, `mvn package`, and `mvn spring-boot:run`. The validated foundation includes Flyway V1, security/CORS, error handling, public ID generation, and `GET /api/v1/public/status`. CRUD, authentication, tracking, and requests remain roadmap work. See `docs/LOCAL_DEVELOPMENT.md` and `docs/ROADMAP.md`.
